...
//===============
// AniList-ID-exact-match fast path.
//
// Some providers (AllAnime, Gojo/Animetsu) embed the AniList ID in their own
// search results. When a candidate carries `extId.anilist === canonical.anilist`
// we have positive proof of identity and can skip the gate/score loop AND the
// details fetch entirely. Same applies to MAL ids since they're 1:1.
//===============
function findExactIdMatch(canonical, candidates) {
    if (!canonical) return null;
    for (const c of candidates) {
        const cid = c && c.extId;
        if (!cid) continue;
        if (canonical.anilist && cid.anilist && String(cid.anilist) === String(canonical.anilist)) return c;
        if (canonical.mal && cid.mal && String(cid.mal) === String(canonical.mal)) return c;
    }
    return null;
}
...
async function selectBest({ canonical, rawCandidates, fetchDetails, opts = {} }) {
    ...
    // ID-exact-match fast path — skip gates AND details fetch.
    const exact = findExactIdMatch(canonical, dedup);
    if (exact) {
        return {
            match: {
                slug: exact.slug,
                ...
                score: 200,
                confidence: "HIGH",
                reasons: [`+200 anilist_id_exact (${exact.extId.anilist || exact.extId.mal})`],
                details: exact
            },
            debug: { reason: "id_exact_match", slug: exact.slug }
        };
    }
...
module.exports = { selectBest, findExactIdMatch };
