...
const { sizeBytesFromStreamCandidate, splitQualityAndSize } = require("../../stream-size");
...
// Per-call timeout deliberately tight: profiling Gojo's /oppai endpoint
// against long-tail episodes (One Piece ep 700+) showed individual server
// responses ranging 1-9s. With 4 servers × 2 source_types = 8 parallel
// calls, the slowest dictates total time when using Promise.all. Capping at
// 5s lets slow servers drop while fast ones still contribute their links.
const OPPAI_TIMEOUT_MS = 5000;
...
function shapeOppai(payload, serverId, isDub) {
    ...
    return sources.filter(s => s && s.url).map(s => {
        ...
        const parsed = splitQualityAndSize(s.quality || "auto");
        const q = parsed.quality && String(parsed.quality).trim() === "master" ? "multi-quality" : parsed.quality;
        return {
            url,
            server: `Gojo/${serverId}`,
            quality: q,
            sizeBytes: sizeBytesFromStreamCandidate(s),
            dub: isDub,
...
async function getStreamsGojo(slug, episode) {
    ...
    // Promise.allSettled (not Promise.all) so a single slow / errored server
    // doesn't blackhole the whole getStreams. Each fetchOppai is wrapped in
    // its own 5s deadline; whatever returns by then contributes streams,
    // whatever doesn't is silently dropped.
    const tasks = [];
